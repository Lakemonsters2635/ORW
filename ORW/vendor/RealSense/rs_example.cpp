...
// Note: the original code should be kept in the same directory as this file and rs_example.h, for 2 reasons:
//
...
void imu_renderer::render(const rs2::motion_frame& frame, const rect& r)
{
	draw_motion(frame, r.adjust_ratio({ IMU_FRAME_WIDTH, IMU_FRAME_HEIGHT }));
}

GLuint imu_renderer::get_gl_handle() { return _gl_handle; }

void imu_renderer::draw_motion(const rs2::motion_frame& f, const rect& r)
{
	if (!_gl_handle)
...
void imu_renderer::multiply_vector_by_matrix(GLfloat vec[], GLfloat mat[], GLfloat* result)
{
	const auto N = 4;
	for (int i = 0; i < N; i++)
	{
...
float2 imu_renderer::xyz_to_xy(float x, float y, float z, GLfloat model[], GLfloat proj[], float vec_norm)
{
...
void imu_renderer::print_text_in_3d(float x, float y, float z, const char* text, bool center_text, GLfloat model[], GLfloat proj[], float vec_norm)
{
 ...
 void  imu_renderer::draw_axes(float axis_size, float axisWidth)
{
...
void imu_renderer::draw_circle(float xx, float xy, float xz, float yx, float yy, float yz, float radius, float3 center, float intensity)
{
	const auto N = 50;
...
void pose_renderer::render(const rs2::pose_frame& frame, const rect& r)
{
	draw_pose(frame, r.adjust_ratio({ IMU_FRAME_WIDTH, IMU_FRAME_HEIGHT }));
}

GLuint pose_renderer::get_gl_handle() { return _gl_handle; }

// Provide textual representation only
void pose_renderer::draw_pose(const rs2::pose_frame& f, const rect& r)
{
	if (!_gl_handle)
		glGenTextures(1, &_gl_handle);
...
GLuint texture::get_gl_handle() { return _gl_handle; }
...
float window::width() const { return float(_width); }
float window::height() const { return float(_height); }
...
void window::show(const rs2::frame& frame, const rect& rect)
{
...
window::operator GLFWwindow* () { return win; }

void window::render_video_frame(const rs2::video_frame& f, const rect& r)
{
	auto& t = _textures[f.get_profile().unique_id()];
	t.render(f, r);
}

void window::render_motion_frame(const rs2::motion_frame& f, const rect& r)
{
	auto& i = _imus[f.get_profile().unique_id()];
	i.render(f, r);
}

void window::render_pose_frame(const rs2::pose_frame& f, const rect& r)
{
	auto& i = _poses[f.get_profile().unique_id()];
	i.render(f, r);
}

void window::render_frameset(const rs2::frameset& frames, const rect& r)
{
...
rect window::calc_grid(rect r, size_t streams)
{
...
std::vector<rect> window::calc_grid(rect r, std::vector<rs2::frame>& frames)
{
...
void draw_pointcloud(float width, float height, glfw_state& app_state, rs2::points& points)
{
	...
	for (int i = 0; i < points.size(); i++)
	{
		if (vertices[i].z)
		{
			...
			if (tex_coords[i].u != 0 || tex_coords[i].v != 0)
			{
				int q = 0;
			}
